...
| `epub_view` 3.2.0 | MIT | Pure Flutter rendering documents Android and Linux support, but it renders publications/pages rather than Barii’ normalized stable bites. | Credible cross-platform viewer, rejected as core. |
...
Barii therefore uses `epubx` only for EPUB container, metadata, and spine
